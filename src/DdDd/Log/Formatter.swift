...
open class Formatter {
    ...
    public convenience init(style: Style) {
        ...
        case .full:
            ...
            components = [ .date(formatter: dateFormatter), .text(" ["), .logLevel, .text("] "), .thread,
                           .text(" ["), .file, .text(":"), .line, .text("] "),
    ...
    public enum Component {
        case text(String), date(formatter: DateFormatter), logger, logLevel, message, function, file, line, thread//, Any(stringForEvent: (event: Event<M>) -> String)
        ...
        public func string<M>(from event: Event<M>) -> String {
            ...
            case .file:
                return  URL(fileURLWithPath: event.file).lastPathComponent
            case .line:
