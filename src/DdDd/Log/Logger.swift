...
public let defaultLogger: Logger = {
    ...
    logger.handlers.append(FileHandler(fileURL:logURL)!)
    return logger
}()
...
public func getLogger(forFile file: String = #file) -> Logger {
    ...
    return Log.getLogger(Logger.KeyPath(components: [ fileURL.lastPathComponent ]))
}
...
{
    Log.getLogger(forFile: file).log(message, error: error, forLevel: logLevel, onceForKey: key, function: function, file: file, line: line)
}
...
public func verbose<M>(_ message: @autoclosure @escaping () -> M, error: Error? = nil, onceForKey key: String? = nil, function: String = #function, file: String = #file, line: Int = #line) {
    Log.log(message, error: error, forLevel: .verbose, onceForKey: key, function: function, file: file, line: line)
}
...
public func debug<M>(_ message: @autoclosure @escaping () -> M, error: Error? = nil, onceForKey key: String? = nil, function: String = #function, file: String = #file, line: Int = #line) {
    Log.log(message, error: error, forLevel: .debug, onceForKey: key, function: function, file: file, line: line)
}
...
public func info<M>(_ message: @autoclosure @escaping () -> M, error: Error? = nil, onceForKey key: String? = nil, function: String = #function, file: String = #file, line: Int = #line) {
    Log.log(message, error: error, forLevel: .info, onceForKey: key, function: function, file: file, line: line)
}
...
public func warning<M>(_ message: @autoclosure @escaping () -> M, error: Error? = nil, onceForKey key: String? = nil, function: String = #function, file: String = #file, line: Int = #line) {
    Log.log(message, error: error, forLevel: .warning, onceForKey: key, function: function, file: file, line: line)
}
...
public func error<M>(_ message: @autoclosure @escaping () -> M, error: Error? = nil, onceForKey key: String? = nil, function: String = #function, file: String = #file, line: Int = #line) {
    Log.log(message, error: error, forLevel: .error, onceForKey: key, function: function, file: file, line: line)
}
...
public func critical<M>(_ message: @autoclosure @escaping () -> M, error: Error? = nil, onceForKey key: String? = nil, function: String = #function, file: String = #file, line: Int = #line) {
    Log.log(message, error: error, forLevel: .critical, onceForKey: key, function: function, file: file, line: line)
}
...
{
    Log.getLogger(forFile: file).tic(andLog: message, error: error, forLevel: logLevel, timerKey: timerKey, function: function, file: file, line: line)
}
...
{
    Log.getLogger(forFile: file).toc(andLog: message, error: error, forLevel: logLevel, timerKey: timerKey, function: function, file: file, line: line)
}
...
/**
 ...
 Every environment variable with prefix `Log` is evaluated as a logger key path and assigned a log level corresponding to its value. Values should match the log level descriptions, e.g. `Debug`.

 Valid environment variable declarations would be e.g. `Log = Debug` or `Log.MyLogger = Verbose`.
 */
public func configureFromEnvironment()
{
    ...
    for (key, value) in environmentVariables where key.hasPrefix(prefix) {
        ...
        let logger = Log.getLogger(keyPath)
        if let logLevel = LogLevel(description: value) {
            logger.logLevel = logLevel
            configurations.append((logger, logLevel))
        } else {
            log("Invalid Log log level '\(value)' for key path '\(keyPath)' in environment variable.", forLevel: .warning)
        }
    }
    if configurations.count > 0 {
        log("Configured Log logging from environment. Configurations: \(configurations)", forLevel: .debug)
    } else {
        log("Tried to configure Log logging from environment, but no valid configuration was found.", forLevel: .warning)
    }
}
...
public final class Logger {
    ...
    /// The root of the logger hierarchy
    @available(*, deprecated: 0.10, message: "Use the global `Log.defaultLogger` instead.") public class func defaultLogger() -> Logger {
        return Log.defaultLogger
    }
    ...
    /**
     ...
     Generally, the logger's key will be the file name and it will be a direct child of the default logger.
     */
    @available(*, deprecated: 0.10, message: "Use the global `Log.getLogger(forFile:)` function instead.") public class func loggerForFile(_ file: String = #file) -> Logger {
        return Log.getLogger(forFile: file)
    }
    ...
    /**
     ...
     A parent-child relationship is established and can be used to set specific settings such as log levels and handlers for only parts of the logger hierarchy.
     */
    @available(*, deprecated: 0.10, message: "Use the global `Log.getLogger()` function instead.") public class func loggerForKeyPath(_ keyPath: Logger.KeyPath) -> Logger {
        return Log.getLogger(keyPath)
    }
...
extension Logger: CustomStringConvertible {
    ...
    public func description(_ keyPathSeparator: String? = nil) -> String {
        var keyPath = self.keyPath(upToParent: Log.defaultLogger)
        if self.root !== Log.defaultLogger {
