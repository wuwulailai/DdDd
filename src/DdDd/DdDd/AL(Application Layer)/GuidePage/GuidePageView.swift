...
public class GuidePageView: UIView {
    ...
    public convenience init?(frame: CGRect, imageNamesGroup: [String], delegate: GuidePageImageCustomized) {
    ...
    /// - Parameter showNewVersion: 是否版本更新时显示
    public convenience init?(frame: CGRect, videoName: String, delegate: GuidePageVideoCustomized) {
    ...
    @objc fileprivate func enterButtonEvent() {
        removeFromSuperview()
        if let onFinishedHandler = onFinishedHandler {
            onFinishedHandler()
        }
    }
